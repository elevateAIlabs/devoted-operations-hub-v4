  ...
  return (
    <>
      ...
      <div className="operations-grid">
        <section className="panel"><SectionHeading eyebrow="Open work" title="Needs Action" />{operations.length ? operations.slice(0, 8).map((item) => <RecordRow key={item.id} item={item} onOpen={props.onOpen} />) : <EmptyState title="No open operations items" text="Capture a lead, project, payroll issue, or fleet follow-up." />}</section>
        <section className="panel"><SectionHeading eyebrow="Readiness" title="Project Readiness" />{projects.map((item) => <button className="readiness-card" key={item.id} onClick={() => props.onOpen(item)}><strong>{item.title}</strong><span>Stage: {item.lifecycleStatus}</span><span>Next: {item.body ? item.body.split("\n")[0] : "Needs information"}</span><b>{item.waitingOn ? `Waiting on ${item.waitingOn}` : item.dueDate ? `Due ${item.dueDate}` : "Deadline not recorded"}</b></button>)}</section>
      </div>
    </>
  );
}
